...
CANARY_ORDER = ["Gaussian Noise", "Uniform Noise", "OOD Natural", "Watermark", "Label Noise"]
...
def setup_axes(
    ax: plt.Axes, log_x: bool = False, max_step: int | None = None, show_xlabel: bool = True
):
    ...
    if not mlflow_sub_df.empty:
        for m_name, label in metric_map.items():
            ...
            if agg.empty:
                continue
            plotted = True
            style = ACCURACY_STYLES.get(
                label, {"color": "#6b7280", "linestyle": "-", "linewidth": 1.5}
            )
            p = ax.plot(
                agg["step"],
                agg["mean"],
...
def generate_combined_plots(
    ...
    fig, axes = plt.subplots(
        2, n_canaries, figsize=(fig_w, fig_h), sharex="col", sharey="row", squeeze=False
    )
...
def main():
    ...
    if not os.path.exists(default_mlflow) and os.path.exists(
    ...
    if os.path.exists(args.mlflow_input):
        ...
        df_mlflow = pd.read_parquet(
            args.mlflow_input, columns=["run_name", "metric_name", "value", "step", "run_id"]
        )
        ...
        print(
            f"Warning: MLflow export '{args.mlflow_input}' not found. Only InfoRMIA data will be plotted."
        )
    ...
    for model_name in all_models:
        model_info_df = (
            df_info[df_info["model_group"] == model_name] if not df_info.empty else pd.DataFrame()
        )
        model_mlflow_df = (
            df_mlflow[df_mlflow["model_group"] == model_name]
