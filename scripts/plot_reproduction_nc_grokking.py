...
def get_polished_name(m):
    ...
    if 'loss' in m and 'train' in m: return 'Train'
    if 'loss' in m and 'test' in m: return 'Test'
    if m == 'eval/weight_norm/total': return 'Total'
    if m.startswith('eval/weight_norm/'): return 'Last Layer'
...
def plot_aggregated(ax, df_plot, metric_names, title, ylabel, twin_ax=None, twin_metrics=None, show_legend=True, vline_x=None):
    if df_plot.empty:
    ...
    for m in metric_names:
        ...
        p = ax.plot(agg_data['step'], agg_data['mean'], label=label_name, color=colors[c_idx % len(colors)])
        color = p[0].get_color()
        if not agg_data['std'].isna().all():
            ax.fill_between(agg_data['step'], agg_data['mean'] - agg_data['std'], agg_data['mean'] + agg_data['std'], color=color, alpha=0.2)
        c_idx += 1
    ...
    if twin_ax is not None and twin_metrics is not None:
        for m in twin_metrics:
            ...
            agg_data = m_data.groupby('step')['value'].agg(['mean', 'std']).reset_index()
            label_name = get_polished_name(m)
            p = twin_ax.plot(agg_data['step'], agg_data['mean'], label=label_name, color=colors[c_idx % len(colors)], linestyle='--')
            color = p[0].get_color()
            if not agg_data['std'].isna().all():
                twin_ax.fill_between(agg_data['step'], agg_data['mean'] - agg_data['std'], agg_data['mean'] + agg_data['std'], color=color, alpha=0.2)
            c_idx += 1
    ...
    if show_legend:
        if twin_ax is not None:
            ...
            ax.legend(lines + lines2, labels + labels2, title='Metric', fontsize='small', loc='best')
        else:
...
def main():
    ...
    for name, group_df in grouped:
        ...
        acc_metrics = [m for m in all_metrics if 'accuracy' in m.lower() and ('train' in m.lower() or 'test' in m.lower())]
        if not acc_metrics:
            acc_metrics = [m for m in all_metrics if 'acc' in m.lower()]
        plot_aggregated(axes[0], group_df, acc_metrics, '(a) Train and Test Accuracy', 'Accuracy', vline_x=first_step_100)
        axes[0].set_ylim(0, 1)
        ...
        if "mse" in loss_function:
            loss_metrics = [m for m in all_metrics if "loss/mse/mean" in m and ("train" in m or "test" in m)]
        else:
            loss_metrics = [m for m in all_metrics if "loss/cross_entropy/mean" in m and ("train" in m or "test" in m)]
            
        plot_aggregated(axes[1], group_df, loss_metrics, '(b) Train and Test Loss', 'Loss', vline_x=first_step_100)
        ...
        plot_aggregated(axes[2], group_df, wn_metrics, '(c) Weight Norm', 'Weight Norm', vline_x=first_step_100)
        ...
        plot_aggregated(ax2a, group_df, nc1_metrics, '(a) NC1 & RNC1', 'NC1', twin_ax=ax2a_twin, twin_metrics=rnc1_metrics, vline_x=first_step_100)
        
        # Subplot (b) NC2 (Features)
        nc2_metrics_noweights = [m for m in all_metrics if m in ('eval/nc/nc2_equinorm', 'eval/nc/nc2_equiangularity', 'eval/nc/nc2_maximal_angle_equiangularity')]
        ax2b = axes[0, 1]
        plot_aggregated(ax2b, group_df, nc2_metrics_noweights, '(b) NC2 (Features)', 'NC2', vline_x=first_step_100)
        
        # Subplot (c) NC2 (Weights Only)
        nc2_metrics_weights = [m for m in all_metrics if m in ('eval/nc/nc2_equinorm_weights', 'eval/nc/nc2_equiangularity_weights', 'eval/nc/nc2_maximal_angle_equiangularity_weights')]
        ax2c = axes[1, 0]
        plot_aggregated(ax2c, group_df, nc2_metrics_weights, '(c) NC2 (Weights)', 'NC2 Weights', vline_x=first_step_100)
        ...
        plot_aggregated(ax2d, group_df, nc3_metrics, '(d) NC3 & NC4', 'NC3', twin_ax=ax2d_twin, twin_metrics=nc4_metrics, vline_x=first_step_100)
        if ax2d_twin:
