# Adapted from https://colab.research.google.com/github/neuralcollapse/neuralcollapse/blob/main/neuralcollapse.ipynb
...
parser.add_argument(
    "--loss_name", type=str, choices=["CrossEntropyLoss", "MSELoss"], default="MSELoss"
)
...
def train(model, criterion, device, num_classes, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader, start=1):
        ...
        pbar.set_description(
            f"Train\t\tEpoch: {epoch} [{batch_idx}/{len(train_loader)} ({100.0 * batch_idx / len(train_loader):.0f}%)] \t"
            f"Batch Loss: {loss.item():.6f} \t"
            f"Batch Accuracy: {accuracy:.6f}"
        )
...
def analysis(graphs, model, criterion_summed, device, num_classes, loader):
    ...
    for computation in ["Mean", "Cov"]:
        ...
        for batch_idx, (data, target) in enumerate(loader, start=1):
            ...
            for c in range(C):
                ...
                if computation == "Mean":
                    ...
                    NCC_scores = torch.stack(
                        [torch.norm(h_c[i, :] - M.T, dim=1) for i in range(h_c.shape[0])]
                    )
            ...
            pbar.update(1)
            pbar.set_description(
                f"Analysis {computation}\tEpoch: {epoch} [{batch_idx}/{len(loader)} ({100.0 * batch_idx / len(loader):.0f}%)]"
            )
...
with mlflow.start_run(run_name=args.run_name) as run, tempfile.TemporaryDirectory() as tmp_dir:
