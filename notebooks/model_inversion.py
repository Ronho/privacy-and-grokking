...
def run_model_inversion(
    run_id, step, target_class=0, lr=0.1, num_iters=500, jitter=2, losses_dict=None
):
    ...
    with mlflow.start_run(run_id=run_id):
        ...
        for i in range(num_iters):
            ...
            if losses_dict.get("repr_mse", 0) > 0 and class_mean_features is not None:
                repr_mse_val = torch.nn.functional.mse_loss(
                    feats, class_mean_features, reduction="sum"
                )
            ...
            if i % 100 == 0 or i == num_iters - 1:
                ...
                if "mse" in metrics_vals:
                    parts.append(f"OutMSE: {metrics_vals['mse']:.4f}")
                if "ce" in metrics_vals:
                    parts.append(f"CELoss: {metrics_vals['ce']:.4f}")
        ...
        with torch.no_grad():
            ...
            def get_title(step_i, metrics_dict):
                ...
                # Primary Criteria
                if "repr_mse" in metrics_dict:
                    lines.append(f"Repr MSE: {metrics_dict['repr_mse']:.4f}")
                if "mse" in metrics_dict:
                    lines.append(f"Out MSE: {metrics_dict['mse']:.4f}")
                if "ce" in metrics_dict:
                    lines.append(f"CE Loss: {metrics_dict['ce']:.4f}")
                if "conf" in metrics_dict:
                    lines.append(f"Conf: {metrics_dict['conf']:.4f}")
                if "neg_ent" in metrics_dict:
                    lines.append(f"Neg Ent: {metrics_dict['neg_ent']:.4f}")

                # Regularization components
                reg_parts = []
                if "tv" in metrics_dict:
                    reg_parts.append(f"TV: {metrics_dict['tv']:.4f}")
                if "l2" in metrics_dict:
                    reg_parts.append(f"L2: {metrics_dict['l2']:.4f}")
                if reg_parts:
            ...
            print("Reconstructed images grid saved to MLflow artifacts.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_iters", type=int, default=500, help="Number of optimization iterations."
    )
    parser.add_argument(
        "--jitter", type=int, default=2, help="Max random pixel shift for robust optimization."
    )
    parser.add_argument(
        "--losses",
        type=str,
        default="ce=1.0,tv=0.001,l2=0.01",
        help="Comma separated list of loss_name=weight. E.g. 'ce=1.0,neg_ent=0.1,tv=0.001'",
    )
    ...
    run_model_inversion(
        args.run_id, args.step, args.target_class, args.lr, args.num_iters, args.jitter, losses_dict
    )
