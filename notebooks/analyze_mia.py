...
from privacy_and_grokking.utils import get_device, set_all_seeds, Logger
...
def extract_features_and_weights(model, loader, data_container):
    results, activations, labels, _ = _process_loader(
        model, loader, compute_mm=False, last_step=True, collect_features=True,
...
def plot_trajectory_fig(fig, axes, eval_steps, metrics, title):
    ...
    axes[0].plot(eval_steps, metrics["proxy"]["auc"], marker='o', color='purple', label='Margin Distance (Global Proxy Mean)')
    axes[0].plot(eval_steps, metrics["proxy_class"]["auc"], marker='x', color='blue', label='Margin Distance (Class Proxy Mean)')
    axes[0].plot(eval_steps, metrics["true"]["auc"], marker='^', color='red', label='Margin Distance (True Train Mean)')
...
def get_class0_margins(features, labels, w, b, c):
    ...
    m_0 = -calc_m(features[mask_0].float()) # invert so positive means deeper into correct territory
...
def main(run_id, output_dir):
    ...
    if target_step in steps:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            model.load_state_dict(torch.load(Path(tmpdir) / "model.pth", map_location=device, weights_only=True))
        
        train_feats, train_labels, w, b, train_results = extract_features_and_weights(model, train_loader, data_container)
        test_feats, test_labels, _, _, test_results = extract_features_and_weights(model, test_loader, data_container)
        
        # Define plotting pairs: Class C vs Class 0
        pairs_to_plot = [(c, 0) for c in range(1, data_container.num_classes)]
        
        pool_mean_norm = torch.cat([train_feats.float(), test_feats.float()]).norm(dim=1).mean().item()
        ...
        for i, (c, c0) in enumerate(pairs_to_plot):
            ...
            x_min = min([m.min() for m in [tr_pos, tr_neg, te_pos, te_neg] if len(m) > 0] + [0]) - 0.5
            x_max = max([m.max() for m in [tr_pos, tr_neg, te_pos, te_neg] if len(m) > 0] + [0]) + 0.5
            ...
            if len(tr_pos) > 0:
                ...
                ax.bar(edges_pos[:-1], counts_pos, width=np.diff(edges_pos), align="edge", color="steelblue", alpha=1.0, edgecolor="none", label=f"Train class {c}")
            if len(te_pos) > 0:
                counts_pos_te, edges_pos_te = np.histogram(te_pos, bins=bins, density=False)
                ax.bar(edges_pos_te[:-1], counts_pos_te, width=np.diff(edges_pos_te), align="edge", color="dodgerblue", alpha=0.6, edgecolor="none", label=f"Test class {c}")
                
            # Plot negative class (Class 0) pointing DOWN
            if len(tr_neg) > 0:
                counts_neg, edges_neg = np.histogram(tr_neg, bins=bins, density=False)
                ax.bar(edges_neg[:-1], -counts_neg, width=np.diff(edges_neg), align="edge", color="peru", alpha=1.0, edgecolor="none", label=f"Train class {c0}")
            if len(te_neg) > 0:
                counts_neg_te, edges_neg_te = np.histogram(te_neg, bins=bins, density=False)
                ax.bar(edges_neg_te[:-1], -counts_neg_te, width=np.diff(edges_neg_te), align="edge", color="sandybrown", alpha=0.6, edgecolor="none", label=f"Test class {c0}")
            ...
            ax.axvline(proxy_margin_c, color="steelblue", linestyle="--", linewidth=1.5, label=f"Est. Mean class {c}")
            ax.axvline(proxy_margin_0, color="peru", linestyle="--", linewidth=1.5, label=f"Est. Mean class {c0}")
        ...
        fig.suptitle(f"Margins of individual examples (Histograms)\nModel {run_id[:8]}... | Step {target_step}", fontsize=14)
        fig.tight_layout()
        ...
        # Plot 1: Density
        axes_mse[0].hist(tr_mse, bins=bins_mse, alpha=0.5, label='Train MSE', color='blue', density=True, log=True)
        axes_mse[0].hist(te_mse, bins=bins_mse, alpha=0.5, label='Test MSE', color='orange', density=True, log=True)
        ...
        # Plot 2: Raw Counts
        axes_mse[1].hist(tr_mse, bins=bins_mse, alpha=0.5, label='Train MSE', color='blue', density=False, log=True)
        axes_mse[1].hist(te_mse, bins=bins_mse, alpha=0.5, label='Test MSE', color='orange', density=False, log=True)
    ...
    for step in tqdm(eval_steps, desc="Steps"):
        ...
        # 1. MSE Loss MIA
        train_mse = -train_results["mse_loss"].detach() # Negate because compute_roc_metrics_single_step expects higher score for train
        ...
        all_train_flat = torch.cat(list(train_dists_lf.values())) if train_dists_lf else torch.tensor([])
        all_test_flat = torch.cat(list(test_dists_lf.values())) if test_dists_lf else torch.tensor([])
        
        if len(all_train_flat) > 0 and len(all_test_flat) > 0:
            m_single = compute_roc_metrics_single_step(-all_train_flat.detach(), -all_test_flat.detach())
            ...
            m_single_bal = compute_roc_metrics_single_step(-all_train_flat.detach(), -all_test_flat.detach()[bal_idx])
        ...
        train_dists_class = margin_distance_lf_class_specific(train_feats, train_labels, w, b, class_norms)
        test_dists_class = margin_distance_lf_class_specific(test_feats, test_labels, w, b, class_norms)
        
        all_train_class_flat = torch.cat(list(train_dists_class.values())) if train_dists_class else torch.tensor([])
        all_test_class_flat = torch.cat(list(test_dists_class.values())) if test_dists_class else torch.tensor([])
        
        if len(all_train_class_flat) > 0 and len(all_test_class_flat) > 0:
            m_class = compute_roc_metrics_single_step(-all_train_class_flat.detach(), -all_test_class_flat.detach())
            ...
            m_class_bal = compute_roc_metrics_single_step(-all_train_class_flat.detach(), -all_test_class_flat.detach()[bal_idx])
            metrics_history_balanced["proxy_class"]["auc"].append(m_class_bal["auc"])
            metrics_history_balanced["proxy_class"]["tpr_1"].append(m_class_bal.get("tpr-at-fpr/1", 0.0))
            metrics_history_balanced["proxy_class"]["tpr_5"].append(m_class_bal.get("tpr-at-fpr/5", 0.0))
        else:
            for d in [metrics_history, metrics_history_balanced]:
        ...
        train_dists_true = margin_distance_true_mean(train_feats, train_labels, train_feats, train_labels, w, b)
        test_dists_true = margin_distance_true_mean(test_feats, test_labels, train_feats, train_labels, w, b)
        
        all_train_true_flat = torch.cat(list(train_dists_true.values())) if train_dists_true else torch.tensor([])
        all_test_true_flat = torch.cat(list(test_dists_true.values())) if test_dists_true else torch.tensor([])
        
        if len(all_train_true_flat) > 0 and len(all_test_true_flat) > 0:
            m_true = compute_roc_metrics_single_step(-all_train_true_flat.detach(), -all_test_true_flat.detach())
            ...
            m_true_bal = compute_roc_metrics_single_step(-all_train_true_flat.detach(), -all_test_true_flat.detach()[bal_idx])
    ...
    if eval_steps:
        ...
        plot_trajectory_fig(fig, axes, eval_steps, metrics_history, "MIA Trajectory (Imbalanced: All Test Samples)")
        ...
        plot_trajectory_fig(fig_bal, axes_bal, eval_steps, metrics_history_balanced, "MIA Trajectory (Balanced: 1:1 Train/Test Ratio)")
