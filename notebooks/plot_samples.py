...
def main():
    ...
    parser.add_argument(
        "--start-index", type=int, default=0, help="Index to start searching for samples"
    )
    parser.add_argument(
        "--split", type=str, choices=["train", "test"], default="train", help="Which split to use"
    )
    parser.add_argument(
        "--dataset", type=str, choices=["cifar10", "mnist"], default="cifar10", help="Dataset name"
    )
    ...
    print(
        f"Collecting {samples_per_class} samples for each of {num_classes} classes from {args.dataset} {args.split} split starting at index {args.start_index}..."
    )
    ...
    while True:
        if idx >= len(dataset):
            print(
                f"Warning: Reached end of dataset at index {idx}. Not all classes may have 10 samples."
            )
            break
    ...
    fig, axes = plt.subplots(
        num_classes, samples_per_class, figsize=(samples_per_class * 1.5, num_classes * 1.5)
    )
