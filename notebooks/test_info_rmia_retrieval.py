...
def parse_args():
    ...
    parser.add_argument("--experiment-name", type=str, required=True, help="Name of the MLflow experiment")
    parser.add_argument("--model-name", type=str, required=True, help="Model name to filter for")
    parser.add_argument("--tracking-uri", type=str, default="http://localhost:5051", help="MLflow tracking URI")
...
def validate_runs_differ_only_by_seed(runs_df):
    ...
    ignore_cols_prefixes = ['tags.', 'metrics.', 'run_id', 'experiment_id', 'status', 'start_time', 'end_time', 'artifact_uri']
    ignore_exact_params = [
        'params.seed', 
        'params.data.seed', 
...
def build_datasets(cfg: TrainConfig):
    ...
    if len(remaining_test_indices) < 1000:
        print(f"Warning: Not enough remaining test samples for population dataset ({len(remaining_test_indices)} < 1000). Using all available.")
...
def get_signals_for_step(step, run_ids, all_x, all_y, pop_x, pop_y, cfg, device="cuda" if torch.cuda.is_available() else "cpu"):
    ...
    for run_idx, run_id in enumerate(tqdm(run_ids, desc=f"Processing runs for step {step}", leave=False)):
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                model = cfg.model(input_dim=cfg.data().input_shape, num_classes=cfg.data().num_classes)
                model.load_state_dict(torch.load(model_path, map_location=device, weights_only=True))
                model.to(device)
                model.eval()
                
                all_signals[:, run_idx] = compute_signals_in_batches(model, all_x, all_y, device, norm_mean, norm_std)
                pop_signals[:, run_idx] = compute_signals_in_batches(model, pop_x, pop_y, device, norm_mean, norm_std)
...
def main():
    ...
    if not validate_runs_differ_only_by_seed(filtered_runs):
        print("Validation failed: Runs differ by more than just seed parameters. Proceeding anyway.")
    ...
    for step in steps:
        ...
        all_signals, pop_signals = get_signals_for_step(step, run_ids, all_x, all_y, pop_x, pop_y, cfg, device)
