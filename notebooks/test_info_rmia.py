...
def main():
    ...
    parser.add_argument(
        "--num-reference-models", type=int, required=True, help="Number of reference models"
    )
    parser.add_argument(
        "--offline-a",
        type=float,
        default=None,
        help="Offline estimation coefficient 'a'. Required unless tuning.",
    )
    parser.add_argument(
        "--tune-a-ref-model",
        type=int,
        default=None,
        help="Index of non-target reference model to tune 'a' on. Overrides --offline-a.",
    )
    parser.add_argument(
        "--signals-dir", type=str, default="signals", help="Directory containing signal files"
    )
    parser.add_argument(
        "--memberships-file",
        type=str,
        default=None,
        help="Path to membership matrix (.npy). If omitted, defaults to first half being members for all models.",
    )
    parser.add_argument(
        "--output-file",
        type=str,
        default="informia_results.npy",
        help="Output file for test statistics",
    )
    ...
    if args.memberships_file:
        ...
        print(
            "Warning: No memberships file provided. Assuming first half of samples are members for all models."
        )
    ...
    print(
        f"Evaluating InfoRMIA for step {args.step} on target model {args.target_model_idx} with a={args.offline_a}..."
    )
    stats = evaluate_informia_from_files(
        step=args.step,
        target_model_idx=args.target_model_idx,
