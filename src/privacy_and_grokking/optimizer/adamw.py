...
class AdamWConfig(OptimizerConfig):
    ...
    def get_tracking_metrics(
        ...
        for p_old, p_new in zip(pre_step_params, model_parameters):
            ...
            step_with_wd = step_diff / (-lr)
            step_with_wd_sq += step_with_wd.pow(2).sum().item()

            # For AdamW, weight decay is decoupled, so we subtract it from the step to isolate the effective grad
            effective_grad = step_with_wd - weight_reg
        ...
        return {
            ...
            "optim/global_step_without_wd_norm": (effective_grad_sq ** 0.5), # without learning rate, without weight decay
            "optim/global_step_norm": (step_with_wd_sq ** 0.5),
