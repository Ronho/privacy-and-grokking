...
def plot_distributions(
    train_data_last, test_data_last, step, cfg, pk, metric_name, filename_prefix
):
    ...
    ax.axvline(
        ...
        linewidth=2,
        alpha=0.8,
        label=f"Members Mean: {train_mean:.3f}",
    )
    ax.axvline(
        train_median,
        color="blue",
        ...
        linewidth=2,
        alpha=0.8,
        label=f"Members Median: {train_median:.3f}",
    )
    ax.axvline(
        test_mean,
        color="orange",
        ...
        linewidth=2,
        alpha=0.8,
        label=f"Non-members Mean: {test_mean:.3f}",
    )
    ax.axvline(
        test_median,
        color="orange",
        ...
        linewidth=2,
        alpha=0.8,
        label=f"Non-members Median: {test_median:.3f}",
    )
...
def plot_evolution(
    train_data_over_time, test_data_over_time, steps, cfg, pk, metric_name, filename_prefix
):
...
def compute_roc_metrics(
    train_data_over_time, test_data_over_time, steps, fpr_rates=[0.01, 0.05, 0.10]
):
    ...
    for step_idx in range(len(steps)):
        ...
        y_true = torch.concatenate(
            [torch.ones(len(train_data_step)), torch.zeros(len(test_data_step))]
        )
...
def plot_roc_curve(train_data_last, test_data_last, step, cfg, pk, filename_prefix):
    ...
    y_true_last = torch.concatenate(
        [torch.ones(len(train_data_last)), torch.zeros(len(test_data_last))]
    ).numpy()
...
def plot_all_attacks_score_distributions(attack_containers, cfg, pk):
    ...
    for idx, cont in enumerate(attack_containers):
        ...
        all_scores = torch.concatenate(
            [torch.tensor(train_scores), torch.tensor(test_scores)]
        ).numpy()
    ...
    fig.suptitle(
        f"Attack Score Distributions at Step {attack_containers[0].steps[-1]}",
        fontsize=14,
        fontweight="bold",
        y=0.995,
    )
    ...
    fig.savefig(
        pk.IMAGE_FOLDER
        / f"all_attacks_score_distributions_step_{attack_containers[0].steps[-1]}.png",
        dpi=150,
    )
    plt.close(fig)
...
def plot_training_and_attack_evolution(attack_containers, cfg, pk):
    ...
    ax2.plot(
        df["step"], df["last_layer_norm"], label="Last Layer Norm", linewidth=2, color="#ec4899"
    )
    ...
    ax4.axhline(
        y=0.5, color="gray", linestyle="--", linewidth=2, alpha=0.5, label="Random Classifier"
    )
...
def plot_combined_models_superplot(models_data, architecture, dataset, pk):
    ...
    for col_idx, (model_name, cfg, attack_containers) in enumerate(models_data):
        ...
        ax.set_title(
            model_name.replace(f"{dataset}_{architecture}_", ""), fontsize=10, fontweight="bold"
        )
