from typing import Literal

import torch

from privacy_and_grokking.loss.regularizer.base import (
    RegularizerType,
    SelfContainedTwoSampleRegularizerConfig,
)
from privacy_and_grokking.metrics.distribution_overlap import soft_distribution_overlap


class OverlapRegularizerConfig(SelfContainedTwoSampleRegularizerConfig):
    """Soft histogram-intersection overlap regularizer.

    Minimises 1 − soft_histogram_overlap(train_losses, val_losses), pushing
    the train loss distribution toward the validation loss distribution.
    """

    name: Literal["overlap"] = "overlap"
    n_bins: int = 50
    sigma: float = 0.05

    def __call__(self) -> RegularizerType:
        validation_set_generator = self.source()

        def regularizer(train_losses: torch.Tensor) -> torch.Tensor:
            val_losses = validation_set_generator(train_losses)
            overlap = soft_distribution_overlap(
                train_losses, val_losses, n_bins=self.n_bins, sigma=self.sigma
            )
            return 1.0 - overlap

        return regularizer
